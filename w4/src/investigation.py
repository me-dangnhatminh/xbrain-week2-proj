...
class InvestigationAgent:
    ...
    def _gather_metrics(self, report: InvestigationReport, service_name: str) -> Dict:
        ...
        metrics = {}
        try:
            result = self.tool_executor.execute("get_service_metrics", service_name=service_name)
            if isinstance(result, dict) and "error" not in result:
                metrics = result
                report.current_status = {
                    "Service": service_name,
                    "P50 Latency": f"{metrics.get('latency_p50_ms', 'N/A')}ms",
                    "P95 Latency": f"{metrics.get('latency_p95_ms', 'N/A')}ms",
                    "P99 Latency": f"{metrics.get('latency_p99_ms', 'N/A')}ms",
                    "Error Rate": f"{metrics.get('error_rate', 'N/A')}",
    ...
    def _gather_sla(self, report: InvestigationReport, service_name: str) -> Dict:
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                sla = result[0] if isinstance(result[0], dict) else {}
    ...
    def _gather_incidents(self, report: InvestigationReport, service_name: str) -> List[Dict]:
        ...
        incidents = []
        try:
            result = self.tool_executor.execute("get_incident_history", service_name=service_name)
    ...
    def _gather_kb_info(self, report: InvestigationReport, service_name: str) -> str:
        ...
        try:
            chunks = self.rag_pipeline.retrieve(
                f"What is the current status and known issues for {service_name}?",
                top_k=3
            )
            if chunks:
    ...
    def _analyze(self, report: InvestigationReport, service_name: str,
        ...
        sla_p99 = sla.get("p99_latency_ms") or sla.get("latency_p99_target_ms")
