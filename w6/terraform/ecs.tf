...
resource "aws_security_group" "ecs_task_sg" {
  ...
  egress {
    ...
    to_port     = 2049
    protocol    = "tcp"
    cidr_blocks = [aws_subnet.data_private.cidr_block]
  }

  # Monitoring API on port 8000 (Internal Task/Service communication)
  egress {
    description = "Internal monitoring API (Task to Task)"
    from_port   = 8000
    to_port     = 8000
    protocol    = "tcp"
    self        = true
