...
interface LightPillarProps {
    ...
    backgroundColor?: string; // e.g. '#ffffff' for light mode, 'transparent' or omit for dark
}
...
const LightPillar: React.FC<LightPillarProps> = ({
    ...
    // Check WebGL support using useMemo instead of useState
    ...
    `;
      ...
      void main() {
        ...
        // Use luminance as alpha so dark/empty areas are transparent
        // This lets the background color (CSS) show through in light mode
        float luminance = dot(finalColor, vec3(0.299, 0.587, 0.114));
        float alpha = clamp(luminance * 3.0, 0.0, 1.0);

        // Blend pillar color over background
        vec3 blended = mix(uBgColor, finalColor, alpha);
        float finalAlpha = max(alpha, uBgAlpha);
    ...
    `;
        ...
        let bgColorVec = new THREE.Vector3(0, 0, 0);
        let bgAlpha = 0.0;
        if (backgroundColor && backgroundColor !== 'transparent') {
            try {
                const bgThree = new THREE.Color(backgroundColor);
                bgColorVec = new THREE.Vector3(bgThree.r, bgThree.g, bgThree.b);
                bgAlpha = 1.0;
            } catch {
